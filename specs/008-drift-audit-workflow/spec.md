...
- **FR-009**: The workflow MUST commit `drift-audit.json` via a session-bundle amend commit per spec 001 FR-014, with a commit message that includes the rubric version and auditor identity. Format: `session bundle amend: <session-id> — drift-audit @ <rubric-version-short-sha>`.
