...
- [ ] T010 [US1] Append a CHANGELOG-equivalent note to `observations/sessions/README.md` recording that summary-workflow v1 (Paths A + B) is now available, with a link to `SUMMARY-WORKFLOW.md` and the spec 009 cross-reference. (No separate CHANGELOG.md exists at `observations/sessions/` currently; use a dated section in README.md or create `observations/sessions/CHANGELOG.md` if parallel structure with `observations/drift-audits/CHANGELOG.md` is preferred.)
