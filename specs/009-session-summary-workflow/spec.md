...
- **FR-003**: The committed `summary.md` MUST contain, at minimum, the five sections mandated by spec 001 FR-012: **session goal or seed** (what the session was for, what question or task was in play); **per-peer contribution summary** (one block per peer, naming each peer's distinct contributions and whether they were taken up by the other peer); **observed coordination patterns** (loops, dominance, interruptions, operator directive-redirects — the qualitative texture the drift-audit does not capture); **notable drift** (cross-referencing `drift-audit.json.verdict` per FR-008); **operator verdict on H1 stability / H1 complementarity / H2 fresh-reader-pass** (per FR-006).
