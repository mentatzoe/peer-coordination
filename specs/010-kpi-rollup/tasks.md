...
- [ ] T025 [P] [US2] Add `tests/peer_session/test_rollup.py` — implement the 10 contract tests from `contracts/rollup-cli-behavior.md` §"Contract tests": happy path 3/3, happy path 3/4 with 1 not-cleared, 1 ambiguous session → draft, missing kpi.json → refuse, missing fresh-reader-audit.json → refuse, stale kpi.json → refuse, session-id collision → refuse, 2-session out-of-range → draft with surface, template excluded, symlink update on second run. Uses fixtures from T017. Depends on T018, T023.
