...
> `allow_from_bots` is a separate allowlist for specific peer bot IDs. Those bots can speak on the bound shared channel without `@mention` and do not need to appear in `allow_from`, but they still do not open a closed session by themselves.
